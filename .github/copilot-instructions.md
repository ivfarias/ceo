# CEO Orchestration System - Github Copilot CLI Instructions

**Note:** Copy this file to the project root (alongside README.md) for Github Copilot CLI to automatically incorporate these instructions.

## Project Overview

This is a multi-agent orchestration system for managing specialized AI agents. The system uses Github Copilot CLI's native subagent architecture with 8 specialized agents for different domains (development, product, QA, marketing, UX, analytics, optimization).
...
# Task procedures in .github/tasks/
ls .github/tasks/
...
2. Follow template at .github/templates/execplan-tmpl.yaml
...
- Reference: `.github/checklists/code-quality-checklist.yaml`
...
- Reference: `.github/checklists/pm-context-checklist.yaml`
...
Ensure agents are in `.github/agents/` directory with proper YAML frontmatter.
...
Some operations require explicit user approval. Check Github Copilot CLI permissions settings.
...
- Agent prompts reference `.github/` files for templates and checklists
- Task procedures are in `.github/tasks/` as YAML workflows
- Agent definitions are in `.github/agents/` as markdown
...
- Review agent files in `.github/agents/` for specific guidance
